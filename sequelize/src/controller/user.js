...
async function SendVerification(id, email, username) {
  const vertoken = generateToken({ id, isEmailVerification: true }, "180s");
...
const userController = {
  login: async (req, res) => {
    try {
      ...
      console.log("test msk: " + user);

      if (!user) {
        throw new Error("username/email/password not found");
      }

      const checkPass = bcrypt.compareSync(password, await user.password);
  ...
  editProfile: async (req, res) => {
    try {
      const { full_name, user_id, bio, username } = req.body;
      let url = null;
      if (req.file) {
